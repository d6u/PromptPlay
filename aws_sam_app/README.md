...
```bash
server_nodejs$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
server_nodejs$ sam local start-api
server_nodejs$ curl http://localhost:3000/
```
...
```bash
server_nodejs$ sam logs -n HelloWorldFunction --stack-name server_nodejs --tail
```
...
```bash
sam delete --stack-name server_nodejs
```
