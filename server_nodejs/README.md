...
1. Build a docker image from a Dockerfile and then add the source of your application inside the Docker image.
   ```sh
   sam build
   ```
2. The second command will package and deploy your application to AWS, with a series of prompts:
   ```sh
   sam deploy --guided
   ```
